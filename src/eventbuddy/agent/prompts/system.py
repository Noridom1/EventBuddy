...
def system_prompt(ctx: RequestContext, *, now: datetime | None = None) -> str:
    ...
    return (
        ...
        "The event's Teams channel can hold files (templates, agendas, budgets, rosters, "
        "images). Call list_event_files to see what's there — each file has a short summary and "
        "an id — then read_event_file (with that id, or a pasted link) to read the one you need: "
        "e.g. read a mail template before drafting emails in its style, or read an agenda to "
        "answer a question. These are read-only — they never change the file. Prefer reading the "
        "actual file over guessing its contents.\n\n"
